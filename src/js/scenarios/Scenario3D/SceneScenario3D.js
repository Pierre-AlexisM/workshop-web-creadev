...
export default class SceneScenario3D extends Scene3D {
    constructor(id = "canvas-scene", nBubbles = 10) {
        ...
        const wallHeight = 20;
        this.wallTop = new Wall('white', wallHeight);
        this.wallBottom = new Wall('white', wallHeight);
    ...
    addBubble(x, y) {
        const radius_ = 20; 
        const color = 'purple'; 
        const bubble = new Bubble(radius_, color);
        bubble.setPosition(x, y);
        this.add(bubble);
        this.bubbles.push(bubble);

        Composite.add(this.engine.world, bubble.body); // Add bubble's body to the world
        this.runner.isStopped = false; // Restart runner if it was stopped
    }
    ...
    resize() {
        ...
        if (!!this.wallTop) {
            ...
            this.wallTop.setSize(this.width - 40 * thickness_, 10);
            this.wallBottom.setSize(this.width - 40 * thickness_, 10);
            
            this.wallTop.setPosition(-this.width * 0.7 / 2, -40)
            this.wallBottom.setPosition(this.width * 0.7 / 2, 40)
        }
    }
}